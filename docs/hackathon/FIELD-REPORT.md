...
- **Sandbox:** uploads require `TRUEFORGE_SANDBOX=1`. A generated Node cleaner runs in a **child process**, then rows land as `source=upload`. Sample pack load stays deterministic without Daytona.
- **Subagents:** after ingest, P&L / Cash / Growth query the cube in parallel (`lib/analysis/subagents.ts`).
